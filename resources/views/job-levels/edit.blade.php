...
@section('page-title', 'Edit Job Level')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b">
            <h2 class="text-lg font-semibold text-gray-800">Form Edit Level Jabatan</h2>
        </div>

        <form action="{{ route('data-master.job-levels.update', $jobLevel->id) }}" method="POST" class="p-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="level_order"
                        value="{{ old('level_order', $jobLevel->level_order) }}"
                        min="1"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('level_order') border-red-500 @enderror"
                        required>
                    @error('level_order')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3"
                    ...
                    placeholder="Deskripsi level jabatan...">{{ old('description', $jobLevel->description) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1"
                        ...
                        {{ old('is_active', $jobLevel->is_active) ? 'checked' : '' }}>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rou
