...
<div class="max-w-2xl">
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b">
            <h2 class="text-lg font-semibold text-gray-800">Form Edit Karyawan</h2>
        </div>

        <form action="{{ route('data-master.employees.update', $employee->id) }}" method="POST" class="p-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" 
                        ...
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('employee_id') border-red-500 @enderror" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="job_level_id" 
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('job_level_id') border-red-500 @enderror"
                        required>
                        <option value="">-- Pilih Job Level --</option>
                        @foreach($jobLevels as $level)
                            <option value="{{ $level->id }}" 
                                {{ old('job_level_id', $employee->job_level_id) == $level->id ? 'selected' : '' }}>
